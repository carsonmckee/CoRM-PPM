...
std::vector<int> flatten(const std::vector<std::vector<int>>& arr){
    std::vector<int> out;
    int d = arr.size();
    int T = arr[0].size();
...
double update_M(double& curr_M, 
    ...
    // a_post = 1 + len(cluster_to_rows)
    // b_post = 1 + psi(v, curr_phi, M=1)
    // return RNG.gamma(a_post, 1/b_post)
    
    double a_post = 1 + n.size();
    double b_post = 1 + psi(v, phi, 1);
...
Samples sample(const std::vector<std::vector<double>>& y_, 
    ...
    int counter = 0;
    for(int j=0; j<d; j++){
        run_lengths.push_back(std::vector<int>(T));
        corm_clusters.push_back(std::vector<int>(T));
        states.push_back(std::vector<int>(T));
        for(int t=0; t<T; t++){
            if(t == 0){
                std::vector<int> n_j(d, 0);
                n_j[j] = 1;
    ...
    std::vector<double> likelihood_sums(d*(T-1), 0.0);
    std::vector<double> log_likelihood_sums(d*(T-1), 0.0);
    std::vector<double> log_likelihood_2_sums(d*(T-1), 0.0);
    ...
    for(int it=0; it<n_samples; it++){
        ...
        for(int j=0; j<d; j++){
            ...
            double val = sample_beta(N + 1, T - N);
            while(val > u_upper){
                val = sample_beta(N + 1, T - N);
            }
            cp_probs[j] = val;
        }
        ...
        // sample auxiliary y
        // should also bet updating state_summary_stats for states[j][T-1]!!
        for(int j=0; j<d; j++){
            state_summary_stats[states[j][T-1]][0] -= y[j][T-1] * y[j][T-1];
            state_summary_stats[states[j][T-1]][1] -= x[j][T-1] * y[j][T-1];
            state_summary_stats[states[j][T-1]][2] -= x[j][T-1] * x[j][T-1];

            yy_sums[j][yy_sums[0].size()-1] -= y[j][T-1] * y[j][T-1];
            xy_sums[j][yy_sums[0].size()-1] -= x[j][T-1] * y[j][T-1];
            xx_sums[j][yy_sums[0].size()-1] -= x[j][T-1] * x[j][T-1];
            
            y[j][T-1] = x[j][T-1]*thetas[states[j][T-1]].first + STD_NORMAL(global_rng()) * std::sqrt(thetas[states[j][T-1]].second);
        ...
        double pred = 1.0;  
        for(int j=0; j<d; j++){
            pred *= dnorm(y_pred[j], thetas[states[j][T-1]].first * x[j][T-1], std::sqrt(thetas[states[j][T-1]].second));
        }
        ...
        if(it >= burn_in){
            std::vector<int> run_flat = flatten(run_lengths);
            std::vector<int> state_flat = flatten(states);
