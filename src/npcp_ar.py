...
def sample(y: np.array, 
    ...
    return HPPC, U, residuals, n_state_store, c_temp, np.array(y_preds)
...
def multivariate_run(y: np.array, 
    ...
    for j in range(d):
        ...
        while error:
            try:
                _, _, _, _, _, pred = sample(y[j, :], n_samples=n_samples, burn_in=burn_in, n_particles=500, params=params, y_pred=y_pred[j])
                error = False
...
if __name__ == "__main__":
    ...
    while error:
        try:
            y_pred_est = multivariate_run(y, 5000, 1000, np.array([1.0, 1.0, 1.0], dtype=np.float64), y_pred)
            error=False
        except Exception as ex:
            print(ex)
            print('retrying')
    np.savetxt(f'eeg_results/y_preds/npcp_{task_id}.csv', np.array([y_pred_est]))
...
if __name__ == "__main__0":
    ...
    file_name = f"SimResults/npcp_{scenario}_{dataset}_{block}.csv"
