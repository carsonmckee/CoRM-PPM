...
if __name__ == "__main__":
    ...
    while error:
        try:
            y_pred_est = multivariate_run(y, 12000, 2000, np.array([1.0, 1.0, 1.0], dtype=np.float64), y_pred)
            error=False
        except Exception as ex:
            print(ex)
            print('retrying')
    np.savetxt(f'eeg_results/y_preds/sticky_ihmm_{task_id}.csv', np.array([y_pred_est]))
...
if __name__ == "__main__0":
    ...
    for i, t_ in enumerate(range(t, t+points_per_task)):
        ...
        while error:
            try:
                y_pred_est = multivariate_run(y, 10000, 2000, np.array([1.0, 1.0, 1.0], dtype=np.float64), y_pred)
    ...
    file_name = f"SimResults/sticky_ihmm_{scenario}_{dataset}_{block}.csv"
